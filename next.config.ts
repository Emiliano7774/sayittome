...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      {
        source:
          "/((?!_next/static|_next/image|downloads/|icons/|favicon|api/view-once/media(?:/|$)|api/admin/anon-express-chats(?:/|$)|api/roleplay-appeal/history(?:/|$)).*)",
        headers: [
          {
            key: "Cache-Control",
      ...
      {
        source: "/api/roleplay-appeal/history",
        headers: [
          {
            key: "Cache-Control",
            value: "private, no-store, max-age=0, must-revalidate",
