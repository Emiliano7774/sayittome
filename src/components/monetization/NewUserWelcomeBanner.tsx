...
const STORAGE_KEY = "sayittome_welcome_poster_v1";
...
export default function NewUserWelcomeBanner({
  ...
  return (
    <div
      className={[
        isModern
          ? "relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-r from-zinc-950 via-black to-zinc-900 p-4"
          : "relative overflow-hidden rounded-[1.4rem] border border-white/10 bg-[#070707] p-4",
        className,
      ].join(" ")}
    >
      <button
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-3 pr-8">
        <div className="flex min-w-0 items-start gap-3">
          <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-violet-500/15 text-violet-300">
            <Sparkles size={18} />
          </div>
          <div>
            <p className="text-xs font-black uppercase tracking-[0.32em] text-violet-300/90">
              {t("apk_welcome_tag")}
            </p>
