...
export default function ShuffleFiltersEmptyState({
  ...
  const titleClass = isModern
    ...
    : "text-lg font-black text-white/35";
  const noteClass = isModern
    ? "mt-4 max-w-lg text-sm font-bold leading-6 text-white/45"
    : "mt-4 max-w-lg text-sm font-bold leading-6 text-white/40";
  const clearButtonClass = isModern
    ...
    : "mt-5 rounded-full border border-[#8C84FF]/30 bg-[#8C84FF]/10 px-5 py-2.5 text-sm font-black text-[#8C84FF]";
  ...
  return (
    ...
    >
      ...
      {errorText ? (
        <p className={`mt-3 font-bold text-white/40 ${isModern ? "" : "max-w-3xl"}`}>
          {errorText}
        </p>
      ) : null}
    </div>
