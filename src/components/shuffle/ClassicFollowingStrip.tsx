...
export default function ClassicFollowingStrip({
  ...
  const sizes = getFollowingScaledSizes(density);

  const sectionClass = `${tokens.followingMt} border-b border-white/10 ${tokens.followingPb}`;
  const labelStyle = { fontSize: `${sizes.labelPx}px` };
  const textStyle = { fontSize: `${sizes.textPx}px` };
  const btnStyle = {
    fontSize: `${sizes.btnPx}px`,
    paddingInline: `${Math.max(10, Math.round(14 * tokens.scale))}px`,
    paddingBlock: `${sizes.btnPy}px`,
  };
        ...
        >
          {profiles.map((profile) => (
            <Link
              ...
              style={{ width: sizes.itemWPx }}
            >
              <div
                className="relative overflow-hidden rounded-full bg-[#141414]"
                style={{
                  width: sizes.avatarPx,
                  height: sizes.avatarPx,
                }}
              >
                {profile.photo ? (
                  <img
                  ...
                  <div className="flex h-full w-full items-center justify-center text-white/30">
                    <UserRound size={Math.max(12, sizes.avatarPx * 0.42)} strokeWidth={1.75} />
                  </div>
                )}
              ...
              <span
                ...
                style={{ ...textStyle, maxWidth: sizes.itemWPx }}
              >
                {profile.username}
              </span>
            </Link>
