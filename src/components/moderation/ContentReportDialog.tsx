...
export default function ContentReportDialog({
  ...
  async function submit() {
    ...
    try {
      ...
      if (evidenceFile) {
        evidenceUrl = await uploadMedia(
          `report_evidence/${Date.now()}_${evidenceFile.name.replace(/[^\w.-]+/g, "_")}`,
          evidenceFile,
        );
      }
  ...
  return (
    <div className="fixed inset-0 z-[999999] flex items-end justify-center bg-black/80 p-4 sm:items-center">
      <div className="w-full max-w-lg rounded-[2rem] border border-white/10 bg-zinc-950 p-6 shadow-2xl">
        ...
        <label className="mb-2 block text-xs font-black uppercase tracking-[0.18em] text-white/45">
          {kind === "perfil_falso" ? t("report_evidence_required_label") : t("report_evidence_optional_label")}
        </label>
        <input
          ref={fileRef}
          type="file"
          ...
          className="mb-4 w-full rounded-2xl border border-white/10 bg-black px-4 py-3 text-sm font-semibold text-white file:mr-3 file:rounded-full file:border-0 file:bg-white file:px-4 file:py-2 file:text-sm file:font-black file:text-black"
        />

        {error ? <p className="mb-3 text-sm font-bold text-red-300">{error}</p> : null}
