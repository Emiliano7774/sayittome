  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-[#0d0d0d]">
      <div className="shrink-0 border-b border-white/10 px-4 py-3">
        <p className="text-sm font-bold text-white/85">Todas las conversaciones</p>
      ...
      >
        <ul>
          {chats.map((chat, index) => {
            ...
            const peerTitle = formatModerationChatListTitle(chat, profileUsername);
            const peerDetail = getModerationChatPeerLabel(chat, profileUsername);

            return (
              <li key={chat.id} className="border-b border-white/6 last:border-b-0">
                <button
                  type="button"
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-bold text-white/90">
                        {peerTitle === "Anónimo" ? "Anónimo" : peerTitle}
                      </p>
                      <p className="mt-0.5 text-[11px] font-bold text-white/35">
                        {index === 0 ? "Última actividad · " : ""}
                        {formatRelativeActivity(chatActivityMs(chat))}
                      </p>
                      <p className="mt-1.5 line-clamp-2 text-xs font-bold text-white/50">
                        {chat.lastMessage || "Sin mensajes"}
                      </p>
                      {peerDetail !== peerTitle ? (
