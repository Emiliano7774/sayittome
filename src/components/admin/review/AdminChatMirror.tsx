...
import { getModerationChatPeerLabel } from "@/lib/moderation/chatReview";
import {
  formatMessageTime,
  ...
  return (
    ...
    >
      <div className="shrink-0 border-b border-white/10 bg-[#0a0a0a] px-4 py-3 md:px-5">
        ...
        <p className="mt-1.5 text-base font-bold md:text-lg">
          <span className="text-violet-300">{profileUsername}</span>
          <span className="mx-2 font-normal text-white/25">habla con</span>
          <span>{peerIsAnon ? "Anónimo" : peer}</span>
        </p>
        <p className="mt-1 text-xs font-bold text-white/35">
          ← {peerIsAnon ? "Anónimo" : peer} · → {profileUsername} ·{" "}
      ...
      <div
        ref={scrollRef}
        className="min-h-0 flex-1 space-y-2.5 overflow-y-auto overscroll-contain px-3 py-4 md:px-5"
      >
        {loading ? (
            ...
            return (
              ...
              >
                <div className={chatBubbleShellClass(true, isProfile)}>
                  <p className={chatBubbleTextClass(true)}>
