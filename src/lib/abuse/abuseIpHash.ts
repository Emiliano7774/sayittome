 ...
 * Production fail-closed when missing. Without secret + proven direct-GCF IP path → PENDING.
 */
...
export function abuseIpRuntimeReady(): {
...
/** Strip brackets / zone id; do not invent validity — isIP decides. */
...
/** Expand IPv6 to canonical 8×4 hex lowercase for stable HMAC. */
...
/**
 * Direct Cloud Functions / Cloud Run host only.
 * Hosting rewrites do not authenticate the proxy — hop counts are unsafe there.
 */
...
/**
 * Exact last X-Forwarded-For hop only (platform-appended on direct GCF/Cloud Run).
 * Do NOT walk left skipping private/invalid entries.
 * If last hop is missing, invalid, or non-public → empty (IP PENDING).
 * Hosting / custom domain → empty.
 */
...
export function getRequestClientIp(req: Request): string {
  return getTrustedRequestClientIp(req);
}

/** @deprecated */
export function normalizeClientIp(raw: string): string {
  return canonicalizeIp(raw) || normalizeClientIpCandidate(raw);
}
